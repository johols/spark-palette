...
export function fetchStatistics() {
  ...
  return function(dispatch){
    ...
    const url = 'https://nobil.no/api/server/search.php?apikey=' + API_KEY + '&apiversion=3&action=search&type=stats_GetSumChargerstations&countrycode=SWE';

    fetchJsonp(url)
    .then(function(response){
      return response.json();
    }).then(function(json){
      dispatch({type:FETCH_STATS_LADDSTATION_SWE_FULFILLED, payload: json});
    }).catch(function(ex){
